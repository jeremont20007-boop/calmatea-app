...
import {
  ...
  type PlatformType, type ScheduleType, type VehicleType, type Vehicle,
} from '@/types'
...
function DaySelector({ selected, onChange }: { selected: string[]; onChange: (days: string[]) => void }) {
...
export function EditVehicleForm({ vehicle }: { vehicle: Vehicle }) {
  ...
  const [isInRemisBase, setIsInRemisBase] = useState(vehicle.is_in_remis_base)
  const [remisBaseName, setRemisBaseName] = useState(vehicle.remis_base_name || '')
  const [neuquenOnly, setNeuquenOnly] = useState(vehicle.neuquen_only)

  // Platforms & schedule
  const [selectedPlatforms, setSelectedPlatforms] = useState<PlatformType[]>(vehicle.platforms)
  const [schedule, setSchedule] = useState<ScheduleType>(vehicle.schedule)

  // Rental availability
  const [availableDays, setAvailableDays] = useState<string[]>(vehicle.available_days || [])
  const [availableHours, setAvailableHours] = useState(vehicle.available_hours || '')

  // Revenue split
  const [revenueSplit, setRevenueSplit] = useState(String(vehicle.revenue_split))
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4 space-y-5 pb-8 max-w-lg mx-auto w-full">
      ...
      <section className="bg-white rounded-3xl p-5 border border-calm-100 space-y-4">
        ...
        <p className="text-xs text-calm-400">
          Define qué porcentaje de las ganancias de la plataforma recibirá el conductor.
        </p>
      ...
      <section className="bg-white rounded-3xl p-5 border border-calm-100 space-y-3">
        ...
        <textarea
          ...
          placeholder="Estado del vehículo, condiciones del acuerdo, requisitos para el conductor..."
          rows={4}
